    ...
    <div class="table-responsive text-nowrap">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            @if (auth()->user()->role === 'admin')
                <a href="{{ url('/admin/riwayat-hafalan/export') }}" class="btn btn-outline-danger mx-2"><i
                        class="ti ti-download fs-6"></i> Download PDF</a>
            @endif
                    ...
                    {{ $item->status === 'belum diperiksa'
                        ...
                        <td>
                            @if ($item->ulang === 'mengulang' || $item->ulang === 'tidak')
                                <span
                                    class="badge 
                    {{ $item->ulang === 'mengulang' ? 'bg-danger' : ($item->ulang === 'tidak' ? 'bg-primary' : 'bg-warning') }}">{{ $item->ulang }}
                                </span>
                            @else
                                -
                            @endif
                            ...
                            @if (auth()->user()->role === 'admin')
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a href="{{ route('admin.pages.detail-hafalan', $item->id) }}"
                                            class="dropdown-item bg-dark text-white">
                                            <i class="ti ti-eye me-1"></i> Detail</a>
                                        <a href="{{ route('admin.edit-hafalan', $item->id) }}"
                                            class="dropdown-item bg-warning text-white">
                                            <i class="ti ti-edit me-1"></i> Edit</a>
                                        <form action="{{ route('admin.daftar-hafalan.destroy', $item->id) }}"
                                            method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" id="delete_confirm" data-confirm-delete="true"
                                                class="dropdown-item bg-danger text-white show_confirm"><i
                                                    class="ti ti-trash me-1"></i> Delete</button>
                                        </form>
                                    </div>
                                </div>
                            @elseif(auth()->user()->role === 'ustad')
                                <a href="{{ route('ustad.pages.detail-hafalan', $item->id) }}"
